...
if (!build_type) {
    ...
    const announced = { '7z': false, 'zip': false };

    const progressInterval = setInterval(() => {
        process.stdout.clearLine();
        process.stdout.cursorTo(0);

        if (progressData['7z'] >= 100 && !announced['7z']) {
            announced['7z'] = true;
            console.log(`${version}.7z created successfully!`);
        }
        if (progressData['zip'] >= 100 && !announced['zip']) {
            announced['zip'] = true;
            console.log(`${version}.zip created successfully!`);
        }

        if (announced['7z'] && announced['zip']) {
